...
open class EncodedDataAlgorithms {
    
    let startAscii: UInt8 = 33
    let endAscii: UInt8 = 126

    private func createVigenereTable()  -> [[Character?]] {
    ...
    public static func encryptText(text: String, key: String) -> String {
        var encryptedText = ""
        let keyLength = key.count
        
        for (index, char) in text.enumerated() {
            let keyChar = key[key.index(key.startIndex, offsetBy: index % keyLength)]
            let encryptedChar = EncodedDataAlgorithms().findIndex(rowText: char, colText: keyChar)
    ...
    private func findIndex(rowText: Character, colText: Character) -> String {
    ...
    public static func decryptText(encryptedText: String, key: String) -> String {
           var decryptedText = ""
           let keyLength = key.count
        let vigenereTable = EncodedDataAlgorithms().createVigenereTable()
        let totalCharacters = Int(EncodedDataAlgorithms().endAscii - EncodedDataAlgorithms().startAscii + 1)
